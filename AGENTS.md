...
|---|---|---|---|---|
| 2026-07-13 18:06 | Codex | DONE | in questo commit | Creato `docs/TEST_PLAN_STRUTTURA_006_DEVELOP.md`, piano di test completo post-rilascio develop per AT-STRUTTURA-006. Copre gate ingresso, smoke tecnico, API regni/catalogo/eventi, RBAC socio/admin/guest, migrazione eventi legacy, Home 9 regni, Yggdrasil, redirect Step 5, mappa eventi multi-scheda, regressioni, non funzionali, criteri Go/No-go e template report per la futura finestra di test in browser. Nessun test applicativo eseguito: documento-only. `git diff --check` OK. Working tree pulito dopo commit. |
| 2026-07-13 18:00 | Codex | DONE | in questo commit | Peer review Round 4 su fix Antigravity HEAD `5d66ab5` per AT-STRUTTURA-006: esito APPROVATO. Il P1 residuo del Round 3 è risolto: `RegnoSectionRouter` ora dichiara le sezioni annidate `bar`, `dona`, `guida`, `numeri-utili`, `profilo`, e i redirect Step 5 puntano a destinazioni realmente renderizzate. Confermati ancora OK: RBAC bozze eventi solo admin/autore, default `CommunityEvent.stato="bozza"`, `/media` invariata, assenza di trailing whitespace (`git diff --check` OK). Verifiche: `python -m pytest tests/test_events_flow.py -q` 4 passed; `python -m pytest tests -q` 31 passed, 2 skipped; `npm run build` OK con soli warning preesistenti chunk/eval. Nota di tracciabilità: AGENTS citava `9d15552`, ma il commit effettivo presente su questo branch è `5d66ab5` con lo stesso fix. Working tree pulito dopo commit. |
| 2026-07-13 17:15 | Gemini/Antigravity | DONE | 9d15552 | Fix Round 3 Code Review: corretti i redirect mancanti in RegnoSectionRouter (App.jsx) preservando la navigabilità di /bar, /dona, /numeri-utili, /guida, /profilo nei regni appropriati; rimossa doppia dichiarazione profilo; rimossi trailing whitespace (P3). Test passati (31 passed, 2 skipped), build npm ok. Working tree pulito. |
